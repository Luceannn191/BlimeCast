...
export default function App() {
  ...
  // Hidden / query-param-unlocked admin nav switcher
  const [showAdminNav, setShowAdminNav] = useState<boolean>(false);
  ...
  useEffect(() => {
    // Secret query param or dynamic unlock checker
    const params = new URLSearchParams(window.location.search);
    if (params.get('admin') === 'true' || params.has('admin') || localStorage.getItem('blimcast_admin_nav_enabled') === 'true') {
      setShowAdminNav(true);
      localStorage.setItem('blimcast_admin_nav_enabled', 'true');
    }
  ...
  const saveOrdersToLocalStorage = (updatedOrders: SublimationOrder[]) => {
    // Keep raw base64 images in React state memory for beautiful rendering in browser session
    setOrders(updatedOrders);
    
    // Automatically sanitize and store lightweight URLs into localStorage to save disk quota
    try {
      ...
      console.error('Gagal mencadangkan pesanan dengan aman ke localStorage:', e);
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white pb-16 w-full max-w-full overflow-x-hidden relative">
      ...
      <footer className="mt-20 border-t border-slate-900 bg-slate-950 py-8 text-xs text-slate-600">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="text-center md:text-left">
            <p 
              className="font-bold text-slate-500 cursor-pointer select-none hover:text-slate-400 active:scale-95 transition-all"
              title="Ketuk 5 kali untuk mengaktifkan Admin mode"
              onClick={() => {
                const nextClicks = footerClicks + 1;
                if (nextClicks >= 5) {
                  setShowAdminNav(true);
                  localStorage.setItem('blimcast_admin_nav_enabled', 'true');
                  alert('Mode Otorisasi Admin Diaktifkan! Panel Admin kini muncul di menu navigasi atas.');
                  setFooterClicks(0);
                } else {
            ...
            >
              © 2026 Blimcast Indonesia. All Rights Reserved.
            </p>
